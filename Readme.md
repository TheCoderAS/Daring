...
- Flight loop pinned to core 0 at ~250 Hz using complementary-filter attitude estimation and PID stabilization. Raw IMU values
  are used so the craft always drives toward true zero attitude instead of a calibrated offset.
...
2. (Optional) To calibrate ESC throttle range on first power-up, set `kCalibrateEscsOnBoot` to `true`, remove props, and power the
   quad so the max-then-min pulse sequence can complete. Return it to `false` for normal use.
