...
## File layout
- `Daring.ino` — task setup, web server, control loop, motor mixing.
- `Imu.h` — lightweight MPU6050 driver with complementary filter.
- `Pid.h` — reusable PID controller implementation.
- `MotorMixer.h` — LEDC PWM setup, ESC arming, motor writes.

## Usage
1. Adjust motor output pins in `kMotorPins` inside `Daring.ino` to match your wiring.
